...
const PatientLobby: React.FC<PatientLobbyProps> = ({
  ...
  const [roomName] = useState(`room-${Math.random().toString(36).substring(2, 11)}`);
  ...
  useEffect(() => {
    ...
    socket.on('INVITE_PATIENT', (data: { roomName: string, consultationId: string }) => {
          ...
          {queueStatus && (
            <div className="mb-6">
              ...
              <p className="text-lg mb-2">
                Estimated wait time: <span className="font-bold">{queueStatus.estimatedWait}</span>
              </p>
              <p className="text-gray-500">
                Please don&apos;t close this window. You&apos;ll be connected with the doctor shortly.
              </p>
            </div>
          )}
          
          <div className="flex justify-center">
            <Button variant="solid" className="bg-red-500" onClick={handleLeaveQueue}>
              Leave Queue
            </Button>
          </div>
        </div>
